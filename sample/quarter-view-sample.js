...
class ChickenDrawer {
  ...
  draw() {
    ...
    let isFacingUs = (a) => (
      (a = normalized(a)), a >= -PI / 36 && a <= (37 * PI) / 36
    );
    let isFacingUsNarrow = (a) => (
      (a = normalized(a)), a >= PI / 12 && a <= (11 * PI) / 12
    );
    angle = normalized(angle);

    let shake = new Vec2(0, animFrame % 20 > 10 ? -2 : 0);
    let bodyCenter = pos.add(new Vec2(0, -16)).add(shake);
    let assCenter = isFacingUsNarrow(angle + PI)
      ? bodyCenter.add(new Vec2(12 * cos(angle + PI), -4))
      : null;
    let headCenter = bodyCenter.add(new Vec2(6 * cos(angle), -16)).sub(shake);
    let beakCenter = isFacingUs(angle)
      ? headCenter.add(new Vec2(12 * cos(angle), 0))
      : null;
    let eyes = [];
    if (isFacingUs(angle - PI / 3))
      eyes.push(headCenter.add(new Vec2(8 * cos(angle - PI / 3), -4)));
    if (isFacingUs(angle + PI / 3))
      eyes.push(headCenter.add(new Vec2(8 * cos(angle + PI / 3), -4)));
...
class Util {
  ...
  static drawArrow(begin, way, brimSize = 20) {
    ...
    if (brimSize !== 0) {
      ...
      [b1, b2].forEach((brim) =>
        line(end.x, end.y, end.add(brim).x, end.add(brim).y)
      );
    }
  ...
  static makeTilemapDrawers(level, root, xAxis, yAxis, viewSize, options) {
    ...
    for (let ty = 0; ty < viewSize; ty++) {
      for (let tx = 0; tx < viewSize; tx++) {
        ...
        let depth = tx + ty;
        drawers.push(
          new QuadDrawer([tvs[0], tvs[1], tvs[2], tvs[3]], "ForestGreen", depth)
        );
        ...
        if (height > 0) {
          ...
          // 上面と底面を繋げて、壁面を作る（X軸側）
          drawers.push(
            new QuadDrawer([tvs[1], bvs[1], bvs[2], tvs[2]], "DarkGreen", depth)
          );
          // 上面と底面を繋げて、壁面を作る（Y軸側）
          drawers.push(
            new QuadDrawer([tvs[2], bvs[2], bvs[3], tvs[3]], "DarkGreen", depth)
          );
        }
      }
    }
...
class Game {
  constructor() {
    ...
    let level = new Level();
    level.heights = [
      4.5, 4.0, 3.5, 5.0, 2.0, 2.0, 3.0, 0.5, 2.0, 2.0, 2.5, 2.0, 1.0, 1.0, 1.5,
      1.0,
    ];
    level.tileXLen = 4;
    level.tileYLen = 4;
  ...
  proc() {
    ...
    if (keyIsPressed) {
      ...
      if (!inputVec.equals(new Vec2(0, 0))) {
        ...
        let futurePos = player.pos.add(
          new Vec2(cos(angle), sin(angle)).mul(1 / 30)
        );
